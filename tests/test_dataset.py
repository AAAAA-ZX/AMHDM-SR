...
from amhdmsrgan.data.amhdmsrgan_dataset import AMHDMSRGANDataset
from amhdmsrgan.data.amhdmsrgan_paired_dataset import AMHDMSRGANPairedDataset


def test_amhdmsrgan_dataset():

    with open('tests/data/test_amhdmsrgan_dataset.yml', mode='r') as f:
        opt = yaml.load(f, Loader=yaml.FullLoader)

    dataset = AMHDMSRGANDataset(opt)
...
def test_amhdmsrgan_paired_dataset():

    with open('tests/data/test_amhdmsrgan_paired_dataset.yml', mode='r') as f:
        opt = yaml.load(f, Loader=yaml.FullLoader)

    dataset = AMHDMSRGANPairedDataset(opt)
    assert dataset.io_backend_opt['type'] == 'disk'  # io backend
    assert len(dataset) == 2  # whether to read correct meta info
