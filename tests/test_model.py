...
from amhdmsrgan.archs.discriminator_arch import UNetDiscriminatorSN
from amhdmsrgan.models.amhdmsrgan_model import AMHDMSRGANModel
from amhdmsrgan.models.amhdmsrnet_model import AMHDMSRNetModel


def test_realesrnet_model():
    with open('tests/data/test_amhdmsrnet_model.yml', mode='r') as f:
        opt = yaml.load(f, Loader=yaml.FullLoader)

    # build model
    model = AMHDMSRNetModel(opt)
    # test attributes
    assert model.__class__.__name__ == 'AMHDMSRNetModel'
...
def test_realesrgan_model():
    with open('tests/data/test_amhdmsrgan_model.yml', mode='r') as f:
        opt = yaml.load(f, Loader=yaml.FullLoader)

    # build model
    model = AMHDMSRGANModel(opt)
    # test attributes
